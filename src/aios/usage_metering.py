...
* **Cost boundary (GAP-3).** ``Project.budget_used`` governs runs that carry
  a MEASURED currency cost. LOCAL runs (``DelegationMode.LOCAL``) record usage
  but no cost *unless* a model price table is configured (Stage 2, env
  ``AIOS_MODEL_PRICING``), in which case ``complete_local_run`` derives a cost
  from that usage. A LOCAL run without a price entry appears in ``run_count`` /
  ``runs_by_status`` / ``no_measured_cost_run_count`` and NEVER in
  ``measured_spend``: visible, but outside the budget scope. A non-zero
  ``no_measured_cost_run_count`` is therefore the meter for that scope
  boundary, not an anomaly. See ``docs/Budget_Cost_Boundary.md``.
...
def budget_reconciliation(
    ...
    *,
    ...
    Scope caveat (GAP-3): ``matches == True`` means accrual and ``budget_used``
    agree -- it does NOT mean every unit of real spend lies inside the budget.
    A LOCAL run whose model has no price entry (Stage 2, env
    ``AIOS_MODEL_PRICING``) carries no measured cost and is outside the budget
    scope by design; read ``no_measured_cost_run_count`` for the size of that
    blind spot.
    """
