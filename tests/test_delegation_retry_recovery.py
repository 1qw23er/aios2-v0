...
The fix scopes the run key to the **execution** (the caller-supplied
``idempotency_key``, which is what identifies one ``execute_task`` invocation):

    H(task_id, agent_id, attempt, execution_key)

This keeps every guarantee that existed before:
* one execution + one attempt -> one row (unchanged, still deterministic)
* attempt N within one execution still differs from attempt N+1
and adds the missing one:
* a *different* execution (new key) of the same task+agent produces a new row.

No migration: the column already exists and stays unique.
"""
...
def test_retry_runs_share_attempt_numbering_without_colliding(
    session: Session, tmp_path: Path
) -> None:
    """Both executions start at attempt=1; uniqueness now comes from the exec key."""
    task = _seed(session)
    ws = _adapter(tmp_path)
    inbox = tmp_path / "in"
    _result_file(inbox, task.id)
    ...
    assert len(runs) == 2
    assert {r.attempt for r in runs} == {1}, "attempt stays per-execution (1..max_retries)"
