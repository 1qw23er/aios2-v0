...
def test_w3b_migration_is_single_head_additive_reversible(
    ...
    # Single head, and it is exactly the W3-B migration.
    cfg = Config(ROOT / "alembic.ini")
    cfg.set_main_option("script_location", str(ROOT / "alembic"))
    script = ScriptDirectory.from_config(cfg)
    assert script.get_heads() == ["20260901_0001_workforce_match_benchmark"]
    ...
    assert version == "20260901_0001_workforce_match_benchmark"
    conn.close()
